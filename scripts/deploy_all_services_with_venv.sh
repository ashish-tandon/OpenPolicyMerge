...
# OpenPolicy Service Deployment Script with Virtual Environment Support
# This script starts ALL services with proper virtual environment activation

set -e

echo "🚀 OPENPOLICY SERVICE DEPLOYMENT WITH VIRTUAL ENVIRONMENTS"
echo "=========================================================="
...
echo "🐍 Starting Python Services with Virtual Environments..."

# Start Policy Service (9001) - Working
...
# Start Search Service (9002) - Fixed metadata issue
echo "  Starting search-service on port 9002..."
cd services/search-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for search-service..."
    python3 -m venv venv
    source venv/bin/activate
...
# Start Auth Service (9003) - Fixed missing database.py
...
# Start Notification Service (9004) - Working
...
# Start Config Service (9005) - Working
echo "  Starting config-service on port 9005..."
cd services/config-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for config-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
...
# Start Health Service (9006) - Working
echo "  Starting health-service on port 9006..."
cd services/health-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for health-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
...
# Start ETL Service (9007) - Fixed entry point
echo "  Starting etl-service on port 9007..."
cd services/etl
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for etl-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
python -m uvicorn src.main:app --host 0.0.0.0 --port 9007 --reload --log-level debug > /tmp/openpolicy_logs/etl-service.log 2>&1 &
cd - > /dev/null
sleep 2

# Start Scraper Service (9008) - Fixed loguru dependency
echo "  Starting scraper-service on port 9008..."
cd services/scraper-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for scraper-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
python -m uvicorn src.main:app --host 0.0.0.0 --port 9008 --reload --log-level debug > /tmp/openpolicy_logs/scraper-service.log 2>&1 &
cd - > /dev/null
sleep 2

# Start Monitoring Service (9010) - Working
echo "  Starting monitoring-service on port 9010..."
cd services/monitoring-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for monitoring-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
...
# Start Plotly Service (9011) - Working
echo "  Starting plotly-service on port 9011..."
cd services/plotly-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for plotly-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
...
# Start MCP Service (9012) - Working
echo "  Starting mcp-service on port 9012..."
cd services/mcp-service
if [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "    Creating virtual environment for mcp-service..."
    python3 -m venv venv
    source venv/bin/activate
    pip install -r requirements.txt
fi
...
# Start API Gateway (Go) - Fixed compilation error
...
echo "==================="
echo "If services fail, check logs: tail -f /tmp/openpolicy_logs/[service-name].log"
echo "Virtual environments are automatically created if they don't exist"
echo "All dependencies are automatically installed"
